...
/**
 * Email service — writes to a Firestore "mail" collection.
 *
 * This is compatible with the official Firebase "Trigger Email from Firestore"
 * extension (https://extensions.dev/extensions/firebase/firestore-send-email).
 *
 * Install the extension and point it at the "mail" collection to activate
 * real email delivery. No code changes required after installing the extension.
 */
...
export const emailTemplates = {
  purchaseRequestSubmitted(data: { requestorName: string; purpose: string; amount: number; approverEmail: string; requestId: string }) {
    return {
  ...
  purchaseRequestApproved(data: { purpose: string; amount: number; requestorEmail: string; requestId: string; comments?: string }) {
    return {
  ...
  purchaseRequestRejected(data: { purpose: string; requestorEmail: string; requestId: string; comments?: string }) {
    return {
      to: data.requestorEmail,
  ...
  purchaseRequestRevisionsNeeded(data: { purpose: string; requestorEmail: string; requestId: string; comments?: string }) {
    return {
      to: data.requestorEmail,
  ...
  expenseSubmitted(data: { purpose: string; amount: number; financeEmail: string; requestId: string }) {
    return {
