...
import { BASE_URL, COLLECTION, ORG_CONFIG, ROLE } from "./constants";
import { Request, VendorSetupRequest } from "./types";
import { getUsersByRole } from "./helpers";

// ─── HTML Email Template ──────────────────────────────────────────────────────

function buildEmailHtml(orgName: string, bodyHtml: string): string {
  return `
<!DOCTYPE html>
<html lang="en">
<head>
...
function requestDetailRows(request: Request): string {
  return `
...
function getOrgName(orgId: string): string {
  return ORG_CONFIG[orgId]?.name ?? orgId;
}
...
export async function sendPreApprovalEmail(
  ...
  const approveUrl = `${BASE_URL}/review?token=${token}&decision=approve`;
  const rejectUrl = `${BASE_URL}/review?token=${token}&decision=reject`;
  const needsEditsUrl = `${BASE_URL}/review?token=${token}&decision=needs_edits`;
  ...
  await sendMail(
    ...
    `[${orgName}] Pre-Approval Request: ${request.id} — ${request.purpose}`,
    buildEmailHtml(orgName, body)
  );
}
 ...
 */
export async function sendApprovedEmail(request: Request): Promise<void> {
  const orgName = getOrgName(request.orgId);
  ...
  await sendMail(
    ...
    `[${orgName}] Pre-Approved: ${request.id} — ${request.purpose}`,
    buildEmailHtml(orgName, body)
  );
}
 ...
 */
export async function sendNeedsEditsEmail(request: Request): Promise<void> {
  const orgName = getOrgName(request.orgId);
  const dashboardUrl = `${BASE_URL}/dashboard`;
  ...
  await sendMail(
    [request.requestorEmail],
    `[${orgName}] Edits Required: ${request.id} — ${request.purpose}`,
    buildEmailHtml(orgName, body)
  );
}
 ...
 */
export async function sendReceiptsReviewEmail(
  request: Request,
  token: string
): Promise<void> {
  const orgName = getOrgName(request.orgId);
  ...
  const reviewers = await getUsersByRole(
    request.orgId,
  ...
  if (reviewerEmails.length === 0) {
    ...
    const admins = await getUsersByRole(request.orgId, ROLE.ADMIN);
    reviewerEmails.push(...admins.map((a) => a.email).filter(Boolean));
  }

  if (reviewerEmails.length === 0) return;
  ...
  await sendMail(
    reviewerEmails,
    `[${orgName}] Receipts Review: ${request.id} — ${request.purpose}`,
    buildEmailHtml(orgName, body)
  );
}
 ...
 */
export async function sendFinalApprovedEmail(request: Request): Promise<void> {
  const orgName = getOrgName(request.orgId);
  const dashboardUrl = `${BASE_URL}/dashboard`;
  ...
  const notifyUsers = await getUsersByRole(request.orgId, ROLE.FINANCE_NOTIFY);
  const notifyEmails = notifyUsers.map((u) => u.email).filter(Boolean);
  const allRecipients = Array.from(
    new Set([request.requestorEmail, ...notifyEmails])
  );

  await sendMail(
    allRecipients,
    `[${orgName}] Final Approved: ${request.id} — ${request.purpose}`,
    buildEmailHtml(orgName, body)
  );
}
 ...
 */
export async function sendRejectedEmail(request: Request): Promise<void> {
  const orgName = getOrgName(request.orgId);
  const dashboardUrl = `${BASE_URL}/dashboard`;
  ...
  await sendMail(
    [request.requestorEmail],
    `[${orgName}] Rejected: ${request.id} — ${request.purpose}`,
    buildEmailHtml(orgName, body)
  );
}
 ...
 */
export async function sendOverageEmail(request: Request): Promise<void> {
  const orgName = getOrgName(request.orgId);
  const bufferAmount = ORG_CONFIG[request.orgId]?.escalationBufferAmount ?? 50;
  ...
  await sendMail(
    reviewerEmails,
    `[${orgName}] Overage Alert: ${request.id} — ${request.purpose}`,
    buildEmailHtml(orgName, body)
  );
}
 ...
 */
export async function sendVendorIntakeEmail(
  vendorSetupRequest: VendorSetupRequest,
  token: string,
  intakeUrl: string
): Promise<void> {
  ...
  await sendMail(
    [vendorSetupRequest.vendorEmail],
    `[${orgName}] Vendor Intake Form — ${vendorSetupRequest.vendorName}`,
    buildEmailHtml(orgName, body)
  );
}
...
export async function sendVendorApprovedEmail(
  ...
  await sendMail(
    [vendorSetupRequest.requestorEmail],
    `[${orgName}] Vendor Approved: ${vendorSetupRequest.vendorName}`,
    buildEmailHtml(orgName, body)
  );
}
...
export async function sendVendorRejectedEmail(
  ...
  await sendMail(
    [vendorSetupRequest.requestorEmail],
    `[${orgName}] Vendor Request Rejected: ${vendorSetupRequest.vendorName}`,
    buildEmailHtml(orgName, body)
  );
}
 ...
 */
export async function sendQBSentEmail(request: Request): Promise<void> {
  const orgName = getOrgName(request.orgId);
  const qbAssistEmails = ORG_CONFIG[request.orgId]?.qbAssistEmails ?? [];
  const dashboardUrl = `${BASE_URL}/dashboard`;

  if (qbAssistEmails.length === 0) return;
  ...
  await sendMail(
    qbAssistEmails,
    `[${orgName}] QB Entry Needed: ${request.id} — ${request.purpose}`,
    buildEmailHtml(orgName, body)
  );
}
...
export async function sendPaymentConfirmationEmail(
  ...
  await sendMail(
    [request.requestorEmail],
    `[${orgName}] Payment Sent: ${request.id} — ${request.purpose}`,
    buildEmailHtml(orgName, body)
  );
}
